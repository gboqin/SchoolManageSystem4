...
namespace SchoolManageSystem.Common.Cache
{
    /// <summary>
    /// 用户信息缓存类
    /// </summary>
    ...
    {
        private static readonly MemoryCache Cache = GlobalSystemWeb.AutofacContainer.Resolve<MemoryCache>();
        ...
        public static void CreateUserCache(UserCacheBo userCache)
        {
            ...
            var sid = userCache.Id.ToString();
            var key = FormatPrefixKey(sid);

            userCache.LoginIp = GlobalSystemWeb.GetClientIp();
            userCache.LoginTime = DateTime.Now;

            var redisClient = GlobalSystemWeb.AutofacContainer.Resolve<RedisClient>();
            redisClient.Set(key, userCache, RedisTimeout);
            AddToMemoryCache(key, userCache);
            //GlobalSystemWeb.HttpContext.Items["uid"] = sid;
            //GlobalSystemWeb.HttpContext.Items["user"] = userCache;
        ...
        public static string GetUidFromClient()
        {
            ...
            string uid = http.Request.Headers["uid"];
            return uid;
        }
